...
manage API keys. See the [bootstrap pattern](#bootstrap-pattern) below.
...
For production BYOC deployments, the recommended pattern is:

1. **Phase 1 (admin key):** An Org Owner creates a bootstrap admin API key via the Console UI (one-time).
   Terraform uses this key to create a project and a project-scoped API key.

2. **Phase 2 (project key):** A separate Terraform configuration uses the project-scoped key
   to provision clusters, databases, and collections.

> **Note:** Provider configuration is static — resolved at plan time, it cannot reference
> resource outputs. Phase 1 and Phase 2 must be separate `terraform apply` runs.

> **Key rotation caveat:** A project-scoped key cannot manage API keys at all (error `96041`).
> Rotating the Phase 2 key always requires going back to Phase 1 with the admin key.
...
- `description` (String) Description of the API key. Defaults to `""`.
...
- `all_volume` (Boolean) Whether to include all volumes in this project. Defaults to `true`.
...
Import an existing API key by its ID. Note that the key_value will be empty
after import — the secret is only available at creation time.
