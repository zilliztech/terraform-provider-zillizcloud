...
func TestClient_ListProjects(t *testing.T) {
	...
	if !reflect.DeepEqual(projects[0].RegionIds, []string{"aws-us-east-1"}) {
		t.Fatalf("regionIds = %#v, want [aws-us-east-1]", projects[0].RegionIds)
	}
}
...
func TestClient_CreateProject(t *testing.T) {
	c, _ := testProjectClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if _, ok := raw["regionIds"]; !ok {
			t.Fatalf("body missing regionIds key: %#v", raw)
		}
		if _, ok := raw["regions"]; ok {
			t.Fatalf("body should not contain regions key: %#v", raw)
		}
...
func TestClient_GetProjectById(t *testing.T) {
	...
	if !reflect.DeepEqual(project.RegionIds, []string{"aws-us-east-1", "gcp-us-west1"}) {
		t.Fatalf("regionIds = %#v, want [aws-us-east-1 gcp-us-west1]", project.RegionIds)
	}
}
...
func TestClient_DeleteProjectById(t *testing.T) {
	c, _ := testProjectClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeProjectResponse(t, w, struct{}{})
	})

	if err := c.DeleteProjectById("proj-1"); err != nil {
		t.Fatalf("DeleteProjectById error: %v", err)
	}
}
