...
func (s *byocOpProjectSettingsDataStore) Describe(ctx context.Context, projectID string, dataPlaneID string) (data BYOCOpProjectSettingsDataModel, err error) {
	request := &zilliz.DescribeByocOpProjectSettingsRequest{
		ProjectId:   projectID,
		DataPlaneId: dataPlaneID,
	}

	tflog.Info(ctx, fmt.Sprintf("Describe BYOC Op Project Settings request: %+v", request))

	response, err := s.client.DescribeByocOpProjectSettings(request)
	if err != nil {
	...
	// data.Instances = InstancesOpConfig{
	// 	CoreVM:        types.StringValue(response.CoreVm),
	// 	FundamentalVM: types.StringValue(response.FundamentalVm),
	// 	SearchVM:      types.StringValue(response.SearchVm),
	// }
