...
func TestProjectResourceReadPopulatesRegionIDsFromAPI(t *testing.T) {
	ctx := context.Background()
	r, schema := testProjectResource(t, func(w http.ResponseWriter, req *http.Request) {
	...
	state := testProjectState(t, ctx, schema, ProjectResourceModel{
		...
		ProjectName: types.StringValue("old-name"),
	...
	if got.ProjectName.ValueString() != "test-project" || got.Plan.ValueString() != "Enterprise" {
		t.Fatalf("state project = %#v", got)
	}
	assertProjectRegionIDs(t, ctx, got.RegionIds, []string{"aws-us-east-1", "gcp-us-west1"})
}

func TestProjectResourceImportPopulatesRegionIDsFromAPI(t *testing.T) {
	ctx := context.Background()
	r, schema := testProjectResource(t, func(w http.ResponseWriter, req *http.Request) {
		if req.Method != http.MethodGet {
			t.Fatalf("method = %s, want GET", req.Method)
	...
	resp := fwresource.ImportStateResponse{State: tfsdk.State{Schema: schema}}
	r.ImportState(ctx, fwresource.ImportStateRequest{ID: "proj-1"}, &resp)
	if resp.Diagnostics.HasError() {
		t.Fatalf("import diagnostics: %s", resp.Diagnostics)
	}
	...
	if got.Id.ValueString() != "proj-1" || got.ProjectName.ValueString() != "test-project" || got.Plan.ValueString() != "Enterprise" {
		t.Fatalf("state project = %#v", got)
	}
...
func TestProjectResourceDeleteSendsAPIRequest(t *testing.T) {
	ctx := context.Background()
	r, schema := testProjectResource(t, func(w http.ResponseWriter, req *http.Request) {
		...
		writeProviderProjectResponse(t, w, struct{}{})
	})
...
func TestProjectResourceDeleteReturnsAPIDiagnostic(t *testing.T) {
	ctx := context.Background()
	r, schema := testProjectResource(t, func(w http.ResponseWriter, req *http.Request) {
		if req.Method != http.MethodDelete {
			t.Fatalf("method = %s, want DELETE", req.Method)
	...
	if resp.Diagnostics[0].Summary() != "Failed to delete project" {
		t.Fatalf("delete summary = %q, want Failed to delete project", resp.Diagnostics[0].Summary())
	}
	if !strings.Contains(resp.Diagnostics[0].Detail(), "Project ID: proj-1") {
		t.Fatalf("delete detail = %q", resp.Diagnostics[0].Detail())
	}
}
...
func assertProjectRegionIDs(t *testing.T, ctx context.Context, value types.Set, want []string) {
	t.Helper()
	var got []string
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("region_ids = %#v, want %#v", got, want)
	}
}
